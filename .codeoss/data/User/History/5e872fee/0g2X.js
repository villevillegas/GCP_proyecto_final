...
app.post('/charges', async (req, res) => {
  try {
    ...
    // Valida que el recharge tenga los campos necesarios (ejemplo)
    if (!recharge.phone || !recharge.amount) {
      return res.status(400).send({ error: 'El recharge debe tener nombre y precio.' });
    }

    // Guarda el recharge en Firestore
    const docRef = db.collection('charges').doc(); // Crea un nuevo documento con ID automático
    await docRef.set(recharge);

    console.log('recharge guardado con ID:', docRef.id);
    res.status(201).send({ id: docRef.id, message: 'recharge guardado correctamente.' });

  } catch (error) {
    console.error('Error al guardar el recharge:', error);
    res.status(500).send({ error: 'Error al guardar el recharge.' });
  }
});
